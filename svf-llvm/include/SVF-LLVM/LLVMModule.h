...
private:
    ...
    /// Same-context extapi import path invoked by buildFunToFunMap().
    ///
    /// Under LLVM 21 LTO, per-function cross-module CloneFunctionInto() is not
    /// robust on merged post-link modules. Import the selected extapi
    /// definitions by cloning/filtering the extapi module inside the same
    /// LLVMContext and merging it into the app module via llvm::Linker.
    void importExtAPIFunctionsViaLink(
        Module* appModule,
        Module* extModule,
